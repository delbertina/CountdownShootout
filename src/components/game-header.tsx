...
const GameHeader = (props: GameHeaderProps) => {
  ...
  const canAnswer = useMemo(
    () => teams.filter((team) => !!team.canAnswer).map((team) => team.id),
    [teams]
  );
  const isAnswering = useMemo(
    () => teams.filter((team) => !!team.isAnswering).map((team) => team.id),
    [teams]
  );
            ...
            .map((team) => (
              <div className="flex flex-row gap-4">
                <ShadedIndicator
                  ...
                  theme={team.theme}
                  isShaded={
                    canAnswer.includes(team.id) &&
                    isAnswering.length === 0 &&
